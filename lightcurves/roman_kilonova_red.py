...
# Red kilonova: plot LSST optical curves only from this time (days) so they are monotonically increasing
LSST_OPTICAL_CUTOFF_DAY = 1.1
    ...
    with plt.rc_context(serif_rc):
        ...
        for band in bands:
            ...
            if band in LSST_BANDS:
                mask = times >= LSST_OPTICAL_CUTOFF_DAY
                t_plot = times[mask]
                m_plot = mags[band][mask]
            (line,) = ax.plot(
                t_plot, m_plot, label=label, lw=2, color=color if color else None
            )
